...
function App() {
	...
	function handleRegister(password, emailUser) {
		...
		auth
			.register(escape(password), emailUser)
			.then(() => {
	...
	function handleLogin(password, emailUser) {
		...
		auth
			.authorize(escape(password), emailUser)
			.then((data) => {
				auth
					.getContent(data)
					.then((res) => {
	...
	React.useEffect(() => {
		...
		Promise.all([api.getInitialUserInfo(), api.getInitialCards()])
			...
			.catch((err) =>
				console.log(`Данные пользователя и карточки не прогрузились. ${err}`)
			)
	}, [])
	...
	return (
		<CurrentUserContext.Provider value={currentUser}>
			{loggedIn && (
				<MenuMobile email={email} signOut={handleSignOut} isShowMenu={isShowMenu} />
			)}
			<Header
				loggedIn={loggedIn}
				locaction={location}
			...
			<ImageZoom isOpen={selectedCard} onClose={closeAllPopups} image={dataImage} />
